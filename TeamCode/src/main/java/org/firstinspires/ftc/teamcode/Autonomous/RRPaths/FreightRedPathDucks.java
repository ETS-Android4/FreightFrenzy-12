...
public class FreightRedPathDucks extends LinearOpMode {
    ...
    public static int x1 = -48, leftX = 10, middleX = 100, rightX = 200, allY = 195;

    public Pose2d pose1 = new Pose2d(x1, 0), score = new Pose2d(-84);

    public static int duckLocation = -1, manualDuckLocation = -1;

    public static double level0 = 1350, level1 = 1500, level2 = 1700, sensorSideOffset, sensorStrightOffset;
...
//        if(manualDuckLocation != -1) slideTicks = manualDuckLocation < 2 ? (manualDuckLocation == 0 ? 0 : level1) : level2;
        ...
        drive.turn(Math.toRadians(121));
        Trajectory toDucks = drive.trajectoryBuilder(drive.getPoseEstimate())
                .strafeTo(new Vector2d(-124,7))
                .build();
        drive.followTrajectory(toDucks);

        drive.leftDuckSpinner.setPower(-1); //Unsure on power
        sleep(3000);
        drive.leftDuckSpinner.setPower(0);

        Trajectory zoom = drive.trajectoryBuilder(drive.getPoseEstimate())
                .splineTo(new Vector2d(-78, 19), Math.toRadians(0))
                .splineTo(new Vector2d(2,26), Math.toRadians(0))
                .build();
